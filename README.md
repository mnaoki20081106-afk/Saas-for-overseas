...
| **手で投稿** | Pinterest の審査が通るまでの逃げ道。1枚あたり **2タップ** で投稿できます。<br>①内容を入れた状態で Pinterest を開く → ②「投稿した」を押して記録 |
| **承認** | GO / STOP を押す。外に出る前の関門 |
| **案件（応募）** | ①英世が選んだ理由を読む → ②応募ページを開いて申し込む → ③発行されたURLを貼る |
| **状態** | 数字を見る。おかしいと感じたら「すべて止める」 |

### iPhone のホーム画面に置く（アプリのように使う）

1. Safari で `https://<サイトのURL>/admin/` を開く
2. 下の **共有ボタン**（□に↑）→ **「ホーム画面に追加」**
3. 追加されたアイコンから開くと、**Safari のバーが消えてアプリのように開きます**

**★ホーム画面から開いた最初の1回だけ、GitHub のトークンをもう一度貼ってください。**
Safari とホーム画面のアプリは、別々の保管場所を使うためです（1回だけです）。
