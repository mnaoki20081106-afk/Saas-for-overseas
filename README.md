...
```bash
npm run co -- status    # 会社の状態を1画面で見る（何をするにもまずこれ）
```

### オーナーの画面 — 代表取締役デスク（`/admin/`）

なおきさんが iPad から開く画面です。**4つのタブだけ**です。

| タブ | 何をするか |
| --- | --- |
| **投稿の確認** | これから出る／すでに出た文章を **眺めるだけ**。承認は要りません。<br>おかしければ、その場で取り消せます（記事の取り下げ・ピンの削除） |
| **承認** | GO / STOP を押す。外に出る前の関門 |
| **案件（応募）** | ①英世が選んだ理由を読む → ②応募ページを開いて申し込む → ③発行されたURLを貼る |
| **状態** | 数字を見る。おかしいと感じたら「すべて止める」 |

**アフィリエイトへの応募は、なおきさんにしかできません。** 本人確認が必要だからです。
AI社員は「どこに・なぜ応募するか」を出典つきで揃えるところまでをやります。
