...
### 3b. なおきさんが取り消したものを読む（★見落とし厳禁）

`status` に「**なおきさんが取り消したもの**」が出ていたら、必ず読んでください。

- **取り下げられた記事** → 理由を読み、同じ書き方を繰り返さないよう一葉に伝える
- **削除依頼が残っているピン** → まだ Pinterest に残っています。
  Pinterest の認証情報が未設定だと自動では消えません。なおきさんに手動削除を頼む

**取り消しは、なおきさんが「これは出したくない」と言った唯一の実データです。**
承認の GO/STOP より強いシグナルとして扱ってください。

**取り消しを AI から実行してはいけません**（鉄のルール／`takedownRequestedAt` は
なおきさんだけが書きます）。消したほうがよいと思ったら、承認依頼を出してください。

### 3c. 投稿の承認を外してよいか、条件を見る

```bash
npm run co -- autonomy
```

オーナーは 2026-08-31 にこう決めました。

> **「最初は A案（毎回GOを押す）。しばらく運用して品質が保証できるようになったら B案を提案して」**

- **「★ すべて満たしました」と出たとき、はじめて提案します。**
  A案/B案の形で、`skills/owner-communication.md` の型に従って出してください。
- **それ以外のときは、提案してはいけません。**
  「そろそろ大丈夫そう」で出さないこと。条件は `co` が数えます。
- 条件が1つでも欠けているのに提案すると、**オーナーの「決めたのに蒸し返された」になります。**

**あなたが GO を外すことはできません。** `config/limits.json` は安全装置なので、
AI が書き換えると `guard.yml` が検査で落とします。外すのはオーナー本人の操作です。
提案が通ったら、手順をオーナーに渡してください。
